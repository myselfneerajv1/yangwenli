/**
 * IndiaMap — India States Choropleth
 *
...
const METRIC_CONFIG: Record<Metric, {
  ...
  amount:    { label: 'Total Spending',  colorRange: ['#dbeafe', '#1d4ed8'], format: formatCompactINR },
...
function getMetricValue(state: SubnationalStateSummary, metric: Metric): number {
  if (metric === 'amount')    return state.total_value_inr ?? 0
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5 text-slate-300">
        <p>Spending: <span className="text-white font-medium">{formatCompactINR(state.total_value_inr ?? 0)}</span></p>
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1 border border-border/40 rounded-md p-0.5">
          <Button
            variant="default"
            size="sm"
            className="h-6 text-xs gap-1.5 px-2 cursor-default"
              ...
              >
                ...
                >
                  <Geographies geography={GEO_URL}>
                    {({ geographies }) =>
                      geographies.map((geo) => {
                        const name      = geo.properties?.NAME_1 as string | undefined
                    ...
                    .map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-2 text-right tabular-nums text-xs">
                            {formatCompactINR(s.total_value_inr ?? 0)}
                          </td>
